...
function doActivate(newConfig, callback) {
    ...
    if (newConfig.authentication && newConfig.authentication.enabled) {
        ...
        if (! newConfig.authentication.url && newConfig.authentication.host && newConfig.authentication.port ) {
            newConfig.authentication.url = "http://" + newConfig.authentication.host + ":" + newConfig.authentication.port;
...
function checkConfig(newConfig, callback) {
    ...
    if(newConfig.authentication && newConfig.authentication.enabled){
        if(newConfig.authentication.type && newConfig.authentication.type === 'oauth2'){
            ...
            for (var i in mandatoryOauth) {
                if (!newConfig.authentication[mandatoryOauth[i]]) {
                    missing.push("authentication."+ mandatoryOauth[i]);
                }
            }
        } else {
            var mandatoryKeystone = ['user', 'password'];
            for (var i in mandatoryKeystone) {
                if (!newConfig.authentication[mandatoryKeystone[i]]) {
                    missing.push("authentication."+ mandatoryKeystone[i]);
                }
            }
        }
    }
